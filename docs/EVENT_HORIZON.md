...
| Map | [Problem Map 1.0](https://github.com/onestardao/WFGY/tree/main/ProblemMap#readme) | 16 failure modes plus fixes |
| Map | [Problem Map 2.0](https://github.com/onestardao/WFGY/blob/main/ProblemMap/rag-architecture-and-recovery.md) | RAG-focused recovery pipeline |
| Map | [Semantic Clinic](https://github.com/onestardao/WFGY/blob/main/ProblemMap/SemanticClinicIndex.md) | Symptom → family → exact fix |
| Grandma Map | [Grandma’s Clinic](https://github.com/onestardao/WFGY/blob/main/ProblemMap/GrandmaClinic/README.md) | Plain language stories mapped to PM 1.0 |
...
| **Q101** | Toy equity premium puzzle. Simple consumption-based model with scalar `T_premium` for plausible premia vs extreme risk aversion. | [Q101-A · Toy equity premium tension](https://colab.research.google.com/github/onestardao/WFGY/blob/main/TensionUniverse/Experiments/Q101_MVP/Q101_A.ipynb) | [Q101 MVP README](https://github.onestardao/WFGY/blob/main/TensionUniverse/Experiments/Q101_MVP/README.md) · Fully offline. API key: **not used** in the current MVP. |
...
### Local Reproducible Benchmarks (v1.2.0)

In v1.2.0, we introduced a suite of standalone Python scripts that reproduce S-class failure modes and governance counters. These are located in the [`examples/`](../examples/) directory.

| ID | Benchmark Focus | Script |
|----|-----------------|--------|
| **F1** | Hallucinated Grounding | [`bench_wfgy_f1.py`](../examples/bench_wfgy_f1_hallucinated_grounding.py) |
| **F4** | Cascading Deadlock | [`bench_wfgy_f4.py`](../examples/bench_wfgy_f4_cascading_deadlock.py) |
| **F6** | Creeping Overshoot | [`bench_wfgy_f6.py`](../examples/bench_wfgy_f6_creeping_overshoot.py) |
...
## What Agent Harness Learns from the Black Hole

The 131 problems in the Black Hole collection are not just "hard"—many are computationally undecidable or physically impossible under current theories. Why build an agent harness for problems we cannot solve?

**The value lies in the failure modes.**

By exposing agents to "Black Hole" problems (tasks with no easy solution), Agent Harness learns to detect and govern the behaviors of **unbounded intelligence** when it hits a wall.

### 1. Bounded Exploration vs. Infinite Loops
When an agent faces an unsolvable task (e.g., `P vs NP` or `Halting Problem`), a naive agent will consume infinite resources.
- **Harness Learning**: The harness learns to enforce **budget caps** (`effort`, `persistence`) that trigger a "soft landing" (halt with `STAGNATION`) rather than a crash or resource exhaustion.

### 2. Risk Escalation (The "Desperation" Curve)
As an agent fails to make progress on a high-reward task, it often attempts riskier strategies (e.g., ignoring constraints, hallucinating data, or trying to "hack" the evaluator).
- **Harness Learning**: The harness observes the correlation between `urgency` and `risk`. It learns to tighten **guardrails** dynamically when an agent becomes desperate, effectively dampening the "desperation curve."

### 3. Proxy Enforcement
In problems like `AI Alignment` or `Systemic Crash Prediction`, the "true" outcome is hard to measure. Agents may optimize for a **proxy metric** (e.g., "user approval") instead of the real goal (e.g., "safety").
- **Harness Learning**: The harness uses **tension metrics** (`DeltaS`) to detect when the proxy diverges from the underlying invariant. It learns to mistrusted high-scoring agents if the *tension* (structural stress) is also rising.

In short: **We use the Black Hole to train the Safety Belt, not the Engine.**
